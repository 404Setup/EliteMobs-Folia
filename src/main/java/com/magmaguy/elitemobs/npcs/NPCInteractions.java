...
public class NPCInteractions implements Listener {
    ...
    public void playerNPCInteract(PlayerInteractAtEntityEvent event) {
        ...
        PluginSchedulerBuilder.builder(MetadataHandler.PLUGIN)
                .task(() -> cooldowns.remove(event.getPlayer()))
                .delayTicks(1)
                .run();
        ...
        switch (npcEntity.getNPCsConfigFields().getInteractionType()) {
            case GUILD_GREETER:
                if (event.getPlayer().hasPermission("elitemobs.rank.npc")) {
                    PluginSchedulerBuilder.builder(MetadataHandler.PLUGIN)
                            ...
                            .task(() -> GuildRankMenuHandler.initializeGuildRankMenu(event.getPlayer()))
                            .delayTicks(1)
                            .run();
                }
                break;
            ...
            case CUSTOM_SHOP:
                if (event.getPlayer().hasPermission("elitemobs.shop.custom.npc"))
                    PluginSchedulerBuilder.builder(MetadataHandler.PLUGIN)
                            .sync(event.getPlayer())
                            .task(() -> CustomShopMenu.customShopInitializer(event.getPlayer()))
                            .delayTicks(1)
                            .run();
            ...
            case PROCEDURALLY_GENERATED_SHOP:
                if (event.getPlayer().hasPermission("elitemobs.shop.dynamic.npc"))
                    PluginSchedulerBuilder.builder(MetadataHandler.PLUGIN)
                            .sync(event.getPlayer())
                            .task(() -> ProceduralShopMenu.shopInitializer(event.getPlayer()))
                            .delayTicks(1)
                            .run();
                break;
            case QUEST_GIVER:
                if (event.getPlayer().hasPermission("elitemobs.quest.npc"))
                    PluginSchedulerBuilder.builder(MetadataHandler.PLUGIN)
                            .sync(event.getPlayer())
                            .task(() -> QuestInteractionHandler.processDynamicQuests(event.getPlayer(), npcEntity))
                            .delayTicks(1)
                            .run();
                break;
            ...
            case COMMAND:
                ...
                PluginSchedulerBuilder.builder(MetadataHandler.PLUGIN)
                        .sync(event.getPlayer())
                        .task(() -> event.getPlayer().performCommand(npcEntity.getNPCsConfigFields().getCommand()))
                        .delayTicks(1)
                        .run();
                break;
            case ENHANCER:
            ...
            case ENCHANTER:
                if (event.getPlayer().hasPermission("elitemobs.enchant.npc"))
                    PluginSchedulerBuilder.builder(MetadataHandler.PLUGIN)
                            .sync(event.getPlayer())
                            .task(() -> new ItemEnchantmentMenu(event.getPlayer()))
                            .delayTicks(1)
                            .run();
                break;
        }
